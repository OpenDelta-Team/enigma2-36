	...
	def getNetworkList(self):
		...
		if not self.oldInterfaceState:
			if not iNetwork.getAdapterAttribute(self.iface, "up"):
				...
				Console().ePopen(("ifconfig %s up" % self.iface))
				driver = iNetwork.detectWlanModule(self.iface)
				if driver == "brcm-wl":
					Console().ePopen("wl up")
		try:
			scanresults = list(Cell.all(self.iface, 10))
	...
	def stopGetNetworkList(self):
		if self.oldInterfaceState is not None:
			if not self.oldInterfaceState:
				...
				Console().ePopen(("ifconfig %s down" % self.iface))
				driver = iNetwork.detectWlanModule(self.iface)
				if driver == "brcm-wl":
					Console().ePopen("wl down")
				self.oldInterfaceState = None
				self.iface = None
...
class Status:
	...
	def iwconfigFinished(self, result, retval, extra_args):
		...
		if ssid is not None and ssid != "off" and ssid != "":
			...
			try:
				scanresults = list(Cell.all(iface, 10))
