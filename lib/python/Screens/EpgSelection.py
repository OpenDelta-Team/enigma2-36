...
class EPGSelection(Screen, HelpableScreen):
		...
		self["EPGFilterActions"] = HelpableActionMap(self, ["EPGFilterActions"],
			{
				...
				"saveTimes": (self.filterValuesAsDefault, _("Use current filter values as default") if config.epg.filter_restore_timespan.value else ""),
			})
	...
	def filterValuesAsDefault(self):
		def save(answer):
			if answer:
				self.original_cfg_filter_start = config.epg.filter_start.value
				self.original_cfg_filter_end = config.epg.filter_end.value
		if self.filtering and config.epg.filter_restore_timespan.value:
